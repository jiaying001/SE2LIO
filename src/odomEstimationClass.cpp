...
OdomEstimationClass::OdomEstimationClass(){
    ...
    edge_map = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
    surf_map = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
    current_edge_points = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
    current_surf_points = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
...
void OdomEstimationClass::addLidarFeature(const pcl::PointCloud<pcl::PointXYZI>::Ptr edge_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr surf_in){
    pcl::PointCloud<pcl::PointXYZI>::Ptr downsized_edge = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::PointCloud<pcl::PointXYZI>::Ptr downsized_surf = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
    ...
    pcl::transformPointCloud(*downsized_edge, *current_edge_points, T_bl); //downsized_edge为源点云，current_edge_points为变换后的点云，T_bl为变换矩阵  
...
void OdomEstimationClass::addEdgeCost(ceres::Problem& problem, ceres::LossFunction *loss_function, double* pose){
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr transformed_edge = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
    ...
    for (int i = 0; i < (int)transformed_edge->points.size(); i++){
        ...
        if (pointSearchSqDis[4] < 1.0)
        {
            ...
            if (saes.eigenvalues()[2] > 3 * saes.eigenvalues()[1]){ 
                ...
                ceres::CostFunction *cost_function = new LidarEdgeFactor(curr_point, point_a, point_b, lidar_param.getEdgeN(),lidar_param.getPlanmotion_rotx(), lidar_param.getPlanmotion_roty(), lidar_param.getPlanmotion_z(),lidar_param.getSigma_uscale());  
...
void OdomEstimationClass::addSurfCost(ceres::Problem& problem, ceres::LossFunction *loss_function, double* pose){
    ...
    T_wb.linear() = Utils::so3ToR(Eigen::Vector3d(pose[0],pose[1],pose[2]));
    T_wb.translation() = Eigen::Vector3d(pose[3],pose[4],pose[5]);
    pcl::PointCloud<pcl::PointXYZI>::Ptr transformed_surf = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>());
    ...
    for (int i = 0; i < (int) transformed_surf->points.size(); i++){
        ...
        if (pointSearchSqDis[4] < 1.0){
            ...
            if (planeValid)
            {
                ceres::CostFunction *cost_function = new LidarSurfFactor(curr_point, norm, negative_OA_dot_norm, lidar_param.getSurfN(),lidar_param.getPlanmotion_rotx(), lidar_param.getPlanmotion_roty(), lidar_param.getPlanmotion_z(),lidar_param.getSigma_uscale());    
                problem.AddResidualBlock(cost_function, loss_function, pose);
                surf_num++;
            }
        }
...
void OdomEstimationClass::updateLocalMap(Eigen::Isometry3d& transform){
    ...
    *surf_map += *transformed_surf;
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr edge_map_temp(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::PointCloud<pcl::PointXYZI>::Ptr surf_map_temp(new pcl::PointCloud<pcl::PointXYZI>());
