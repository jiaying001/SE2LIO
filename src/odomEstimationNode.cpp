    ...
    while(true){
        ...
        if(last_synchronization_time == 0.0){
            ...
            while(!imu_buf.empty() && imu_buf.front()->header.stamp.toSec() < lidar_time)
                imu_buf.pop();
            pcl::PointCloud<pcl::PointXYZI>::Ptr edge_points_in(new pcl::PointCloud<pcl::PointXYZI>());
            pcl::PointCloud<pcl::PointXYZI>::Ptr surf_points_in(new pcl::PointCloud<pcl::PointXYZI>());
            ...
            fusionOdometry.header.frame_id = "world"; 
...
void odom_estimation(){
    while (true){
        ...
        for (sensor_msgs::ImuConstPtr& imu_msg: message_syn_buf[0].first){
            ...
            dt_arr.push_back(dt);
            acc_arr.push_back(Eigen::Vector3d(imu_msg->linear_acceleration.x, imu_msg->linear_acceleration.y, imu_msg->linear_acceleration.z)); //position
            gyr_arr.push_back(Eigen::Vector3d(imu_msg->angular_velocity.x, imu_msg->angular_velocity.y, imu_msg->angular_velocity.z)); //rotation
        }
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber sub_imu = nh.subscribe<sensor_msgs::Imu>("/imu/data", 100, imuCallback); //imu_correct
