...
void LaserMappingClass::noiseFilter(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out){
    pcl::CropBox<pcl::PointXYZI> crop_box_filter;
    crop_box_filter.setMin(Eigen::Vector4f(0,-50,2.0, 1.0));
    crop_box_filter.setMax(Eigen::Vector4f(50,50,20, 1.0));
    crop_box_filter.setNegative(false);    
    pcl::PointCloud<pcl::PointXYZI>::Ptr pc_without_ceiling(new pcl::PointCloud<pcl::PointXYZI>());
	...
	pcl::PointCloud<pcl::PointXYZI>::Ptr down_sized_pc(new pcl::PointCloud<pcl::PointXYZI>());
...
void LaserMappingClass::updateCurrentPointsToMap(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, const Eigen::Isometry3d& pose_current){
	...
	for (int i = 0; i < (int)transformed_pc->points.size(); i++)
	{
		...
		//filtering points 
	for(int i=currentPosIdX-lidar_param.getMapCellWidthRange();i<currentPosIdX+lidar_param.getMapCellWidthRange()+1;i++){
		for(int j=currentPosIdY-lidar_param.getMapCellHeightRange();j<currentPosIdY+lidar_param.getMapCellHeightRange()+1;j++){
			for(int k=currentPosIdZ-lidar_param.getMapCellDepthRange();k<currentPosIdZ+lidar_param.getMapCellDepthRange()+1;k++){
